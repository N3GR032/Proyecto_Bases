...
<div class="main" class="container" style="background-color:#f1f1f1">
	 ...
	 <th></th>
 <div>
	<a href="agregar_cliente.php"><button type="text" class="agregar">Agregar</button></a>
	</div>
<table>
		...
		<tr>
			...
			<form  id="form2" name="form2" method="POST" action="clientes.php">

				<h3 align="left" >cliente a buscar</h3>
				...
				<?php 
						...
						if(is_numeric( $_POST["buscar"])==false)
							{
									$query = "SELECT * FROM cliente WHERE nombre_cli LIKE '%".$_POST["buscar"]."%' OR apellidos_cli LIKE '%".$_POST["buscar"]."%' "; 
							}
							else{
									$bus=(int)  $_POST["buscar"];
									$query = "SELECT * FROM cliente WHERE cedula_cli=$bus"; 
							}
				...
				<table>
					...
					<?php while($row = pg_fetch_array($consulta)){ ?>
						<tr>
							<th><input type="text" value = "<?php echo $row['cedula_cli'] ?>"readonly></th>
							<th><input type="text" value = "<?php echo $row['nombre_cli'] ?>"readonly></th>
							<th><input type="text" value = "<?php echo $row['apellidos_cli'] ?>"readonly></th>
							<th><input type="text" value = "<?php echo $row['fecha_nacimiento_cli'] ?>"readonly></th>
							<th><input type="text" value = "<?php echo $row['telefono_cli'] ?>"readonly></th>
							<th><input type="text" value = "<?php echo $row['direccion_cli'] ?>"readonly></th>
							<th><a href="actualizar_cliente.php?cedula_cli=<?php echo $row['cedula_cli'] ?>"><button type="submit">Editar</button></a></th>
							<th><a href="eliminar_cliente.php?cedula_cli=<?php echo $row['cedula_cli'] ?>"><button type="button" class="cancelbtn">Eliminar</button></a></th>
